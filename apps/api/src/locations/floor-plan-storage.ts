...
export interface FloorPlanObjectStorage {
  putObject(input: {
    readonly key: string;
...
export class S3PrivateFloorPlanStorage implements FloorPlanObjectStorage {
  ...
  private async request(
    ...
    if (configuration === undefined)
      throw new Error("Private floor-plan storage is not configured.");

    const endpoint = new URL(configuration.endpoint);
    const basePath = endpoint.pathname.replace(/\/$/u, "");
    const pathname = `${basePath}/${encodeURIComponent(configuration.bucket)}/${encodedPath(key)}`;
    ...
    if (!response.ok) {
      ...
      throw new Error(`Private floor-plan storage returned ${String(response.status)}: ${detail}`);
    }
    return response;
  }
}
