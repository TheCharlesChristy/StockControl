...
Packets D2 (schema and seed) and D3 (stock engine) are done. What exists today is the runtime
foundation, the full stock schema, a seeded demo dataset, and the stock rules from requirements
section 3 with their tests. Authentication, API routes, and the real screens are packets D4 onwards,
so the web shell still shows placeholder sections.
...
5. Load the demo dataset with `pnpm db:seed`.
6. Run development processes with `pnpm dev`.

PostgreSQL listens on `localhost:5432`. It is the only service the demo needs.

### Demo data

`pnpm db:seed` clears the business tables and rebuilds them, so it is safe to re-run. It creates
three users — one per role, all with the password `demo-password`:
...
Alongside them it seeds around 235 catalogue items, 18 store locations, 3 open jobs with their job
sites, and roughly 600 transactions across the last 45 days. The dataset is generated by running the
real stock engine, so every seeded balance is explained by the transactions next to it rather than
being an invented number.
